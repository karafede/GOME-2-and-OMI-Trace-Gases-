%%%%%%%%%%%%%%% read he5 files from OMI/AURA %%%%%%% NO2 Level 3 (L3) %%%%%%%% 

function Read_he5_NO2(FILE_NAME_a,output_directory,NO2_date,NO2_text,NO2_OMI_text)
...
%DATAFIELD_NAME = '/HDFEOS/GRIDS/ColumnAmountNO2/Data Fields/ColumnAmountNO2CloudScreened';  %%%%NO2 vertical column density, screened for CloudFraction < 30%NO2 vertical column density, screened for CloudFraction < 30%
...
%strong_negative = data > -0.5e15;
%data = data(strong_negative);  %%%%%% filter data > than -0.5e+15 (for tracking ship emission purposes)
...
[NO2_OMI] = [LAT,LON,data_Vect];

cd(output_directory);
WWTitle = strcat(NO2_date,' WW');
EUTitle = strcat(NO2_date,' EU');
UKTitle = strcat(NO2_date,' UK');
saveas(fig_WW,WWTitle, 'jpg');
saveas(fig_EU,EUTitle, 'jpg');
saveas(fig_UK,UKTitle, 'jpg');
save(NO2_text,'OMI_AURUN','-ASCII'); %%%% greatsaving ASCII format (AURUN MERGED data from OMI data %%% open with Textpad)
save(NO2_OMI_text,'NO2_OMI','-ASCII');  %%%% txt files for GLOBAL OMI data     
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
